...
// initialize gpio pin used for user button
...
void TIM_INT_Init()
{
    ...
    // TIM5 initialization overflow every 500ms
    // TIM5 by default has clock of 84MHz
    ...
    // TIM5 interrupt is most important (PreemptionPriority and
    // SubPriority = 0)
    NVIC_InitTypeDef NVIC_InitStruct;
...
// resets flags for battery monitoring
void flagReset()
{
	below_10 = 0;
	below_5 = 0;
...
void UpdateMotors()
{
	...
	// initialize patterns
	uint8_t bottom_pattern = 0x00;
	uint8_t middle_pattern = 0x00;
	uint8_t top_pattern = 0x00;
	...
	for(int i=0; i<MAX_ACTIVE_MOTORS; i++)
	{
		...
		else if((min > 1000) && (min < 5000))
		{
			bottom_row = bottom_row | index;
		}
	}

	// pattern cycle used to flip bits
	...
	top_pattern = top_row;
	middle_pattern = middle_row;
	bottom_pattern = bottom_row;

	// check if object recognition is requested
	if(object_recognition){
		if((object_array[1] & patternCycle) == 0x00)
		{
			top_pattern = top_row & 0xf8;		// 0b11111000
			middle_pattern = top_row & 0xf8;
			bottom_pattern = top_row & 0xf8;
		}

		if((object_array[2] & patternCycle) == 0x00)
		{
			top_pattern = top_row & 0xe7;		// 0b11100111
			middle_pattern = top_row & 0xe7;
			bottom_pattern = top_row & 0xe7;
		}

		if((object_array[3] & patternCycle) == 0x00)
		{
			top_pattern = top_row & 0x1f;		// 0b00011111
			middle_pattern = top_row & 0x1f;
			bottom_pattern = top_row & 0x1f;
		}
	}

	vibrate(top_pattern);
	vibrate(middle_pattern);
	vibrate(bottom_pattern);
}

void CheckVoltage(uint16_t adc_val)
{
  // check if battery voltage is less than 10%
  ...
  // check if battery voltage is less than 5%
